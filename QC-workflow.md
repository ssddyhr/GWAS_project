...
```
This command will create the file GWA-data.het
...
```
plink --bfile GWA-QC --remove wrong_het_missing_values.txt --make-bed --out GWAQC

```

### Relatednesss (Identification of duplicated or related individuals)

To identify duplicated or related individuals i will calculate an identity by descent (IBD) matrix.
```
plink --bfile GWA-QC --indep-pairwise 500kb 5 0.2 --out GWA-QC

plink --bfile GWA-data_sflt_mflt_hflt --extract GWA.prune.in --genome --min 0.185 --out GWA
ls -lht
```
Then i remove them
```
